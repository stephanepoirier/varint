...
inline bool CompressedSet::find(unsigned int target) const {
  ...
  if (sizeOfCurrentNoCompBlock != 0) {
    ...
    if (baseListForOnlyCompBlocks.size() == 0 ||
        target > baseListForOnlyCompBlocks[baseListForOnlyCompBlocks.size() - 1]) {
      int i;
      for (i = 0; i < sizeOfCurrentNoCompBlock; ++i) {
        if (currentNoCompBlock[i] >= target) break;
      }
      if (i == sizeOfCurrentNoCompBlock) return false;
  ...
  if (baseListForOnlyCompBlocks.size() > 0) {
    ...
    int index = binarySearchInBaseListForBlockThatMayContainTarget(
...
CompressedSet::Iterator::Iterator(const CompressedSet* const parentSet)
    : iterDecompBlock(DEFAULT_BATCH_SIZE, 0) {
  set = parentSet;
...
unsigned int CompressedSet::Iterator::docID() { return lastAccessedDocId; }

int CompressedSet::Iterator::advanceToTargetInTheFollowingCompBlocks(int target,
                                                                     int startBlockIndex) {
  // searching from the current block
  int iterBlockIndex = binarySearchInBaseListForBlockThatMayContainTarget(
  ...
  int offset = binarySearchForFirstElementEqualOrLargerThanTarget(&(iterDecompBlock[0]), 0,
                                                                  DEFAULT_BATCH_SIZE - 1, target);
  assert(offset >= 0);
  if (offset < 0) {
...
unsigned int CompressedSet::Iterator::Advance(unsigned int target) {
  ...
  if (sizeOfCurrentNoCompBlock > 0) {  // if there exists the last decomp block
    if (iterBlockIndex == compBlockNum ||
        (baseListForOnlyCompBlocksSize > 0 &&
         target > set->baseListForOnlyCompBlocks[baseListForOnlyCompBlocksSize - 1])) {
      offset = binarySearchForFirstElementEqualOrLargerThanTarget(
          &(set->currentNoCompBlock[0]), 0, sizeOfCurrentNoCompBlock - 1, target);
  ...
  if (baseListForOnlyCompBlocksSize > 0 &&
      target <= set->baseListForOnlyCompBlocks[baseListForOnlyCompBlocksSize - 1]) {
...
// searching the right block from the current block to the last block
#ifdef PREFIX_SUM
      lastAccessedDocId =
          advanceToTargetInTheFollowingCompBlocksNoPostProcessing(target, iterBlockIndex);
#else
      ...
      if (target <= set->baseListForOnlyCompBlocks[iterBlockIndex]) {
        ...
        if (offset == DEFAULT_BATCH_SIZE) {
          printf(
              "Error case 3: Impossible, we must be able to find the target %d in the block, "
              "lastAccessedDocId: %d , baseListForOnlyCompBlocks[%d]\n",
