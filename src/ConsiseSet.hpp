...
class OneFillExpander : WordExpander {
  ...
  ~OneFillExpander() {}
  bool hasNext() { return current < lastInt; }
  virtual bool hasPrevious() { return current > firstInt; }
  virtual int next() {
    if (!hasNext()) throw new NoSuchElementException();
    current++;
    if (!set.simulateWAH && current == exception) current++;
    return current;
  }
...
class BitIterator {
  ...
  bool hasNext() { return nextIndex <= set.lastWordIndex || exp->hasNext(); }
  ...
  int next() {
    while (!exp->hasNext()) {
      if (nextIndex > set.lastWordIndex) throw new NoSuchElementException();
      nextWord();
    }
    return exp->next();
  }
  ...
  void skipAllBefore(int element) {
    while (true) {
      ...
      if (exp->hasNext() || nextIndex > set.lastWordIndex) return;
      nextWord();
    }
  }
};
...
int ConciseSet::getLiteral(unsigned int word) {
  ...
  return isZeroSequence(word) ? (ALL_ZEROS_LITERAL | literal) : (ALL_ONES_LITERAL & ~literal);
}
...
bool ConciseSet::add(int e) {
  ...
  // range check
  if (e < MIN_ALLOWED_SET_BIT || e > MAX_ALLOWED_INTEGER) throw IndexOutOfBoundsException(e);
  ...
  for (int i = 0; i <= lastWordIndex && blockIndex >= 0; i++) {
    ...
    if (isLiteral(w)) {
      ...
      if (blockIndex == 0) {
        // bit already set
        if ((w & (1 << bitPosition)) != 0) return false;
        ...
        if (!simulateWAH) {
          ...
          if (bitCount >= MAX_LITERAL_LENGHT - 2) break;
        } else {
          if (containsOnlyOneBit(~w) || w == ALL_ONES_LITERAL) break;
        }
      ...
      if (simulateWAH) {
        if (isOneSequence(w) && blockIndex <= getSequenceCount(w)) return false;
      } else {
        // if we are at the beginning of a sequence, and it is
        // a set bit, the bit already exists
        if (blockIndex == 0 && (getLiteral(w) & (1 << bitPosition)) != 0) return false;

        // if we are in the middle of a sequence of 1's, the bit already exist
        if (blockIndex > 0 && blockIndex <= getSequenceCount(w) && isOneSequence(w)) return false;
      }
...
void ConciseSet::appendLiteral(int word) {
  ...
  if (word == ConciseSetUtils.ALL_ZEROS_LITERAL) {
    ...
    if (lastWord == ConciseSetUtils.ALL_ONES_LITERAL) {
      ...
      words[lastWordIndex] =
          ConciseSetUtils.SEQUENCE_BIT | 1 | ((1 + Integer.numberOfTrailingZeros(~lastWord)) << 25);
    } else {
      words[++lastWordIndex] = word;
    }
  } else {
  ...
  if (isLiteral(lastWord)) {
    if (fillType == 0 && lastWord == ALL_ZEROS_LITERAL) {
      ...
      if (fillType == 0 && containsOnlyOneBit(getLiteralBits(lastWord))) {
        ...
        words[lastWordIndex] =
            SEQUENCE_BIT | length | ((1 + Integer.numberOfTrailingZeros(~lastWord)) << 25);
      } else {
