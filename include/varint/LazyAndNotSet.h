...
class LazyAndNotSetIterator : public Set::Iterator {
  ...
  public:
    LazyAndNotSetIterator(std::shared_ptr<Set::Iterator> baseIter,
                          std::shared_ptr<Set::Iterator> notIter) {
      _baseIter = baseIter;
      _notIter = notIter;
    ...
    unsigned int docID() { return _currID; }
    ...
    unsigned int Advance(unsigned int target) {
      ...
      if (_currID == NO_MORE_DOCS) return _currID;
